...
export default function Enquiries() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    setError('')
    const { data, error: err } = await supabase.from('enquiries').select('*').order('created_at', { ascending: false })
    if (err) setError('Failed to load enquiries.')
    setEnquiries(data ?? [])
    setLoading(false)
  }, [])
          ...
          <div key={row.id} style={{
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
                ...
                <a href={`https://wa.me/${toWaPhone(row.phone ?? '')}`} target="_blank" rel="noopener noreferrer" style={{ color: '#25D366', fontSize: '0.82rem', textDecoration: 'none' }}>{row.phone}</a>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'flex-end' }}>
              ...
              <a
                href={`https://wa.me/${toWaPhone(row.phone ?? '')}?text=Hi%20${encodeURIComponent(row.name ?? '')}!%20This%20is%20Skinora%20responding%20to%20your%20enquiry.`}
                target="_blank"
